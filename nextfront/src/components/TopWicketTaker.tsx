    ...
    <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
      ...
      <div className="space-y-3">
        {wicketTakers.length === 0 && (
          <p className="text-slate-400 text-sm">No bowlers available.</p>
        )}

        {wicketTakers.map((bowler) => (
          <div key={bowler.playerId} className="flex items-center gap-3">
            <img
              src={bowler.image || "/placeholder.svg"}
              alt={bowler.playerName}
            ...
            <div className="flex flex-col">
              <div className="flex flex-row items-center">
                ...
                <p className="text-sm text-zinc-300 font-light">{team}</p>
              </div>
